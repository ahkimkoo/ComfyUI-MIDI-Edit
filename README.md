...
- **CT-Transformer 智能拆句**：当用户歌词句子数少于原曲时，使用 AI 标点恢复模型在自然语言边界处智能拆分，而非粗暴按字数切割。每次只切一刀（句号优先 → 逗号），切点选在靠近中间的位置，切完后去掉标点，下一轮继续拆最长的句子直到够数
...
3. 如果句子数少于原曲 section 数，使用 **CT-Transformer AI 标点恢复模型**智能拆分长句：
   - 每轮选最长的句子，AI 加标点后**只切一刀**
   - 切点优先级：句号 > 逗号，选靠近中间的位置
   - 切完后去掉所有标点，得到两份干净歌词
   - 重复直到句子数够用；AI 无法断句时从中间硬拆
...
当新歌词句子数少于原曲 section 数时，AI 标点恢复模型自动在自然语言边界处拆分，每次只切一刀：

原曲 4 个 section：`春 天 花 开 满 园 香` / `夏 日 蝉 鸣 绿 荫 长` / `秋 风 落 叶 金 满 地` / `冬 雪 纷 飞 白 茫 茫`

新歌词（仅 2 句）：`明月几时有把酒问青天` / `不知天上宫阙今夕是何年`

第 1 轮：拆最长句 `不知天上宫阙今夕是何年` → AI 加标点 → `不知天上宫阙，今夕是何年` → 靠中间找逗号 → 切成 `不知天上宫阙` + `今夕是何年` → 共 3 句

第 2 轮：继续拆最长句 → AI 加标点 → 靠中间找句号/逗号 → 切一刀 → 最终 4 句，匹配 4 个 section
